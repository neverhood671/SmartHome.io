...
function initPage() {
    ...
    initFilter(), initCircleRange(), initVerticalScroll(), initPopup(), initScroll("scenario_arrow", "scenario_card", "scenario_cards"), 
...
function initScroll(e, t, r) {
    document.querySelectorAll(`.${e}`).forEach(e => {
        e.addEventListener("click", e => {
            var o, c = e.currentTarget.classList.contains("next") ? -1 : 1, n = getElementWidth(document.querySelector(`.${t}`)), l = getElementWidth(document.querySelector(`.${r}`)), a = Math.floor(l / n) * n, s = /\.*translateX\((.*)px\)/i.exec(document.querySelector(`.${t}`).style.transform);
            s = null == s ? 0 : parseFloat(s[1]), ((o = s + c * a) > 0 || Math.abs(o) >= document.documentElement.clientWidth) && (o = 0), 
...
function initCircleRange() {
    ...
    addListenerMulti(document.querySelector(".circle"), "mousemove touchmove", e => {
        if (is_dragging) {
            ...
            var r, o = document.querySelector(".circle").offsetWidth / 2 + getOffset(document.querySelector(".circle")).left, c = document.querySelector(".circle").offsetHeight / 2 + getOffset(document.querySelector(".circle")).top, n = e.pageX || t.pageX, l = c - (e.pageY || t.pageY), a = o - n;
            if ((r = (r = Math.atan2(l, a) * (180 / Math.PI)) - 90) < 0 && (r = 360 + r), (r = Math.round(r)) < 140 || r > 220) {
...
function getOffsetRect(e) {
    var t = e.getBoundingClientRect(), r = document.body, o = document.documentElement, c = window.pageYOffset || o.scrollTop || r.scrollTop, n = window.pageXOffset || o.scrollLeft || r.scrollLeft, l = o.clientTop || r.clientTop || 0, a = o.clientLeft || r.clientLeft || 0, s = t.top + c - l, i = t.left + n - a;
    return {
        top: Math.round(s),
        left: Math.round(i)
    };
