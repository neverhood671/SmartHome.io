...
function initPage() {
    ...
    addMouseoverMousoutEventListeners(".card", "card_hover"), initCircleRange(), initPopup(), 
    initScroll("scenario_arrow", "scenario_card", "scenario_cards"), initScroll("devices_arrow", "device_card", "device_cards");
}

function initScroll(e, t, r) {
    document.querySelectorAll(`.${e}`).forEach(e => {
        e.addEventListener("click", e => {
            var o, c = e.currentTarget.classList.contains("next") ? 1 : -1, n = getElementWidth(document.querySelector(`.${t}`)), s = getElementWidth(document.querySelector(`.${r}`)), a = Math.floor(s / n) * n, l = /\.*translateX\((.*)px\)/i.exec(document.querySelector(`.${t}`).style.transform);
            l = null == l ? 0 : parseFloat(l[1]), ((o = l + c * a) > 0 || Math.abs(o) >= document.documentElement.clientWidth) && (o = 0), 
...
function initCircleRange() {
    ...
    addListenerMulti(document.querySelector(".circle"), "mousemove touchmove", e => {
        if (is_dragging) {
            ...
            var r, o = document.querySelector(".circle").offsetWidth / 2 + getOffset(document.querySelector(".circle")).left, c = document.querySelector(".circle").offsetHeight / 2 + getOffset(document.querySelector(".circle")).top, n = e.pageX || t.pageX, s = c - (e.pageY || t.pageY), a = o - n;
            if ((r = (r = Math.atan2(s, a) * (180 / Math.PI)) - 90) < 0 && (r = 360 + r), (r = Math.round(r)) < 140 || r > 220) {
                document.querySelector(".dot").setAttribute("style", "transform: rotate(" + r + "deg)");
                let e = Math.round(r > 180 ? 2 * (r - 180) / 15 - 14 : 10 + 2 * r / 15);
                document.querySelector(".debug").innerHTML = e > 0 ? "+" + e : e;
            }
        }
    });
}
...
function getOffsetRect(e) {
    var t = e.getBoundingClientRect(), r = document.body, o = document.documentElement, c = window.pageYOffset || o.scrollTop || r.scrollTop, n = window.pageXOffset || o.scrollLeft || r.scrollLeft, s = o.clientTop || r.clientTop || 0, a = o.clientLeft || r.clientLeft || 0, l = t.top + c - s, i = t.left + n - a;
    return {
        top: Math.round(l),
        left: Math.round(i)
    };
